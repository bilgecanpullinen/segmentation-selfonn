...
from semseg.utils import AverageMeter, colorEncode, accuracy, intersectionAndUnion, setup_logger, parse_devices
...
def evaluate(segmentation_module, loader, cfg, gpu_id, result_queue):
    ...
    for batch_data in loader:
        ...
        result_queue.put_nowait((acc, pix, intersection, union, area_pred, area_lab, end))
...
def main(cfg, gpus):
    ...
    while processed_counter < num_files:
        if result_queue.empty():
            continue
        (acc, pix, intersection, union, area_pred, area_lab, end) = result_queue.get()
